...
## SMC 结构重写验证(v1.72,标准 BOS/CHoCH on 固定摆点)

适用:`mql5/SMC_OrderFlow_Indicator_v1.72_zig.mq4`
说明:在缠论已规范的固定摆点上,用 bias 状态机做标准 BOS/CHoCH;**上游(摆点/缠论/趋势引擎)保持运行不变**,仅 SMC 结构显示(structure_zones)改为标准重建。
...
**缠论隔离回归(必须):** 对比重写前后截图,**摆点连线、分型、笔、趋势指示器显示完全不变**(因上游一行未动)。

**参照(辅):** chart.html 看趋势/方向同向即可(数量/位置因摆点不同可不等)。

**回退:** 改用 v1.71 文件;或注释掉 `DetectStructureBreaksSMC(...)` 调用(旧 `DetectStructureBreaks` 仍在运行,会恢复旧结构显示)。
